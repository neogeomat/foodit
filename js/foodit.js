...
$countrySelect.change(function () {
  ...
  switch ($country) {
    case "Germany":
      $citiesSelect.empty();
      // debugger;
      Object.keys($citiesList.Germany).forEach(function (city) {
        $citiesSelect.append(
...
$citySelect.change(function () {
  ...
  let $city_long = parseFloat($(this).find(":selected").attr("data-long"));

  if (marker != undefined) {
    map.removeLayer(marker);
  }
  marker = L.marker([$city_lat, $city_long]);
  marker.addTo(map);
  ...
  $.get(url, function (data) {
    ...
    data.forEach(function (item) {
      item = item.split("@");
      $suburbSelect.append(
        `<option value="${item[0]}"data-lat="${item[1]}"data-long="${item[2]}">${item[0]}</option>`
      );
    });
  });
});
